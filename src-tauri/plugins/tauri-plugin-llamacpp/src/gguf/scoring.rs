...
async fn compute_score_result(
    ...
    let gguf = if is_mlx || request.model_path.starts_with("http://") || request.model_path.starts_with("https://") {
        None
    } else {
        match read_gguf_metadata_internal(request.model_path.clone()).await {
            Ok(meta) => Some(meta),
            Err(err) => {
                log::warn!("Hub scoring could not read GGUF metadata for {}: {}", request.model_path, err);
                None
            }
        }
    };
