...
import type { CatalogModel, ModelQuant, ModelScore, ModelScoreStatus } from '@/services/models/types'
...
type ModelScoreState = {
  ...
  fetchModelScore: (model: CatalogModel, variant?: ModelQuant) => Promise<ModelScore | undefined>
  reset: () => void
}
  ...
  persist(
    (set, get) => ({
      ...
      fetchModelScore: async (model: CatalogModel, variant?: ModelQuant) => {
        ...
        if (cachedScore && cachedScore.status !== 'loading') return cachedScore

        set((state) => ({
          scores: {
            ...state.scores,
            [modelKey]: { status: 'loading', estimated_tps: 0 },
          },
        }))
        ...
        const scoreRequest = getServiceHub()
          ...
          .then((scoreResult) => {
            ...
            return {
              ...
              updated_at: scoreResult.updated_at ?? Math.floor(Date.now() / 1000),
            }
          })
          .then((score) => {
            set((state) => ({ scores: { ...state.scores, [modelKey]: score } }))
            return score
          })
          .catch((error) => {
            ...
            set((state) => ({ scores: { ...state.scores, [modelKey]: failedScore } }))
            return failedScore
          })
          .finally(() => pendingScoreRequests.delete(modelKey))
    ...
    { name: localStorageKey.modelScores, storage: createJSONStorage(() => localStorage) }
  )
)
